...
func (v *View) GetFile(uri lsp.DocumentURI) *File {
	v.mu.Lock()
	f, found := v.files[uri]
	if !found {
		f = &File{
...
func (v *View) TypeCheck(uri lsp.DocumentURI) (*packages.Package, error) {
	v.mu.Lock()
	defer v.mu.Unlock()
...
func (v *View) TypeCheckAtPosition(uri lsp.DocumentURI, position lsp.Position) (*packages.Package, *ast.File, token.Pos, error) {
	v.mu.Lock()
	defer v.mu.Unlock()
